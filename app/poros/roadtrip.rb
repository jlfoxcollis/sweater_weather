class Roadtrip
  ...
  def daily_forecast(weather, data)
    ...
    {"temperature": selected[:temp][:day], "conditions": selected[:weather].first[:description]}
  end

  def t_time(data)
    return "impossible route" unless data[:info][:statuscode] != 402 && data[:route][:realTime] != 10000000
